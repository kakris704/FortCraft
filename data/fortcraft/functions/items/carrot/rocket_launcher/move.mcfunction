...
## 移動
    ...
    #パーティクル
        ...
        execute if score @s FN.BulletTime matches 4.. run particle cloud ~ ~ ~ 0.1 0.1 0.1 0.05 1 force 

    #キル
        scoreboard players add @s FN.BulletTick 1
        scoreboard players add @s FN.BulletTime 1
        execute if score @s FN.BulletTime matches 800.. run function fortcraft:items/carrot/rocket_launcher/explosion
