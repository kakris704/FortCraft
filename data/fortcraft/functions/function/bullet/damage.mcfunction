...
## 処理
    # 計算
        execute unless score $Damage FN.Temporary <= @s FN.Shield as @a if score @s FN.PlayerNumber = @e[tag=FN.Bullet,sort=nearest,limit=1] FN.PlayerNumber at @s anchored eyes run loot spawn ^ ^-0.25 ^1 loot fortcraft:show_damage
        execute if score $Damage FN.Temporary <= @s FN.Shield as @a if score @s FN.PlayerNumber = @e[tag=FN.Bullet,sort=nearest,limit=1] FN.PlayerNumber at @s anchored eyes run loot spawn ^ ^-0.25 ^1 loot fortcraft:show_damage_shield
